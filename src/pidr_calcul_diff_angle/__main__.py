...
@click.option("-n", default=1, help="Seuil pour valider ou non la modélisation par défaut, sa valeur est 1")
def check(ref, mes,n) -> None:
    # vérifie si la modélisation est correcte en affichant l'erreur moyenne sur les valeurs d'azimut et de hauteur
    statistique.modelisation_correcte(ref, mes, n)
...
@click.option("-s", default=200, help="Valeur d'erreur quadratique moyenne pour corriger les données, si elle n'est pas renseignée il n'y a pas de correction")
def decalage(ref, mes,s) -> None:
    # calcul le décalage d'azimut entre le fichier de reference et celui de mesure
    statistique.donne_moyenne_decalage_azimut_corrige(ref, mes, s)


@calibration.command(help="Exécuter l'ensemble des fonctions")
@click.option("-i", default=5, help="Intervalle entre deux données,"" valeur en minutes parmi 5,10,15,20,30,60")
@click.option("-a", default=1, help="Année de donnée à récupérer via SunEarthTools, 1970-2050")
@click.option("-m", default=1, help="Mois de donnée à récupérer via SunEarthTools, 1-12")
@click.option("-j", default=1, help="Jour de donnée à récupérer via SunEarthTools, 1-31")
